...
(1) The associated `TermCreator` is instructed to create a `Term`, and the `{Event} event` that started the interaction is forward to the `TermCreator`.
...
(3) The `TermCreator` notifies listeners that a new `Term` has been created by firing `{Emitter} termCreatedEmitter`. The `event` that was provided by the `TermCreator` is provided as an argument to `termCreatedEmitter.emit3`.
...
(6) The `event` that started the interaction is propagated to the `TermNode`'s drag listener (a subtype of `TermDragListener`) and interaction with the newly-created `Term` begins.
